...
type DataPoluentesAtmosfericosMunicipio struct {
...
type PoluentesAtmosfericosMunicipio struct {
	Atualizacao time.Time                            `json:"data_atualizacao"`
	Fonte       string                               `json:"fonte"`
	Data        []DataPoluentesAtmosfericosMunicipio `json:"data"`
}

func RunnerPoluentesAtmosfericosMunicipios(emissores PoluentesAtmosfericosEmpresas) {
	runnerName := "Poluentes Atmosféricos - Municipios"

	l := logger.Instance()

	index := PoluentesAtmosfericosMunicipio{}
	now := time.Now()
	index.Atualizacao = now
	index.Fonte = emissores.Fonte

	file_path := "./data/ambientais/municipios_emissao_poluentes.json"
	fileNameOutputCSV := "./data/ambientais/municipios_emissao_poluentes.csv"

	s3KeyCSV := "ambientais/municipios_emissao_poluentes.csv"
	s3KeyJSON := "ambientais/municipios_emissao_poluentes.csv"

	ordenado := make(map[string]DataPoluentesAtmosfericosMunicipio)
	...
	for _, v := range emissores.Data {
		i := fmt.Sprintf("%s%s%s%s", v.Referencia, v.Estado, v.Municipio, v.Poluente)
		d := DataPoluentesAtmosfericosMunicipio{
